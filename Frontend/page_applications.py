...
class ApplicationsPage(Page):
    def __init__(self, session_token, account_type, user_id):
        ...
        self.body_header = QLabel("Pending Account Applications")
        ...
        for app in applications:
            ...
            accept_button.clicked.connect(lambda _, app_id=app["id"], sug_wg=sug_widget: self.accept_application(app_id, sug_wg))
            ...
            reject_button.clicked.connect(lambda _, app_id=app["id"], sug_wg=sug_widget: self.reject_application(app_id, sug_wg))
            sug_layout.addWidget(reject_button)
            sug_layout.setAlignment(reject_button, Qt.AlignmentFlag.AlignCenter)

    def accept_application(self, app_id, app_wg):
        try:
            internal_data = {"submission_id": app_id}
            headers = {"Content-Type": "application/json"}

            response = requests.post(f"http://127.0.0.1:5000/auth/upgrade/approve?user_id={self.user_id}", json=internal_data, headers=headers)
    ...
    def reject_application(self, app_id, app_wg):
        try:
            internal_data = {"submission_id": app_id}
            headers = {"Content-Type": "application/json"}

            response = requests.post(f"http://127.0.0.1:5000/auth/upgrade/deny?user_id={self.user_id}", json=internal_data, headers=headers)
            response.raise_for_status()
    ...
    def fetch_applications(self):
        ...
        try:
            ...
            response = requests.get(f"http://127.0.0.1:5000/auth/upgrade/requests?user_id={self.user_id}", headers=headers)
            response.raise_for_status()
